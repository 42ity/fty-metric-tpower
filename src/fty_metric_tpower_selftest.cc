...
// Tests for stable public classes:
    { "fty_metric_tpower_server", fty_metric_tpower_server_test, true, true, NULL },
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "metricinfo", NULL, true, false, "metricinfo_test" },
    { "calc_power", NULL, true, false, "calc_power_test" },
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_METRIC_TPOWER_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_tpower_private_selftest (verbose, item->subtest);
#endif // FTY_METRIC_TPOWER_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            fty_metric_tpower_private_selftest (verbose, test->subtest);
#endif // FTY_METRIC_TPOWER_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
